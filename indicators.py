...
from utils import default_indicator, format_billions, format_date, format_trillions, format_trillions_compact, validate_date
...
def indicator_fed_balance() -> dict:
    ...
    def _run() -> dict:
        ...
        date, value = latest_point(series)
        return _build("연준 대차대조표", "주간", "FRED (WALCL)", "백만 달러 기준 · 저빈도 지표", format_trillions(value), "안정", date)
...
def indicator_rrp() -> dict:
    ...
    def _run() -> dict:
        ...
        return _build("RRP", "일간", "FRED (RRPONTSYD)", "백만 달러 기준", format_trillions_compact(value), status, date)
...
def optional_mmf_total() -> dict:
    ...
    def _run() -> dict:
        ...
        date, value = latest_point(series)
        return _build("MMF 총자산", "주간", "FRED (WMMFSL)", "백만 달러 기준 · 저빈도 지표", format_trillions(value), "안정", date)
